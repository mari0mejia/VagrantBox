...
Pixmap::Pixmap()
  : Game_object(0.5, 0.5, 0.5, GPL::PIXMAP), m_pixmap_data(nullptr), m_tried_to_read_current_file(false)
{
  Register("filename", std::string(""));
  // if the default pixmap has yet to be created, create it
  if (!default_pixmap_data)
  {
    default_pixmap_data = std::make_shared<Pixmap_data>(default_width,
                                          default_height,
                                          default_data
                                         );
  }
