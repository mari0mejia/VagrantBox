...
Member variables
 ...
 Solution:
  ...
  Access:
      ...
      + if var doesn't match member type: GPL::Type of attribute
...
{
  private:
    static std::unordered_map<std::type_index, GPL::Type> typemap;
  protected:
    const std::type_index proptype;
    static GPL::Type GetGPLType(std::type_index ti)
    { return typemap.at(ti); }
  public:
    GPL::Type GetGPLType() const
...
class Property : public Prop
{
  ...
  public:
    ...
    //If T does not match property type, throw GPL::Type of property
    T& PropertyVal()
    {
...
{
  public:
    Game_object(double red, double green, double blue, GPL::Type got);
    virtual ~Game_object();
    ...
    /////////////
    // Return the GPL::Type of the attribute
    // + Throws out_of_range if name is unregistered 
    /////////////
    GPL::Type attribute_type(const std::string& name) const
    {
    ...
    // + Throws out_of_range if name is unregistered 
    // + Throws GPL::Type of attribute if var doesn't match member type
    /////////////
    template<typename T>
