...
1.  Copy the files from ``p3`` to ``p4``:
...
.. topic:: Game objects
    ...
    .. cpp:function:: GPL::Type attribute_type(const std::string& name)
    ...
    * Throws the ``GPL::Type`` of the attribute as an exception if the type of ``var`` does not match the type of the named ``Game_object`` attribute.
    ...
    * ``var`` may be an ``int``, ``double``, or ``std::string`` (or ``Animation_code*`` used in future project).
    ...
    .. code-block:: cpp
       :emphasize-lines: 1,4,7
        ...
        GPL::Type param_type=game_object_ptr->attribute_type("red");
...
2.  Derive a ``Game_attribute_constant`` class from ``Constant``. Because a game object is always used with one of its attributes, ``Game_attribute_constant`` has two pieces of private data:
    ...
    .. note::
        ...
        Because the constructor invokes ``GPL::Type attribute_type(const std::string& name)``, an ``out_of_range`` exception may be thrown.
...
11. ``Parameter`` objects are allocated and initialized in the action of the ``parameter:`` production. Parameter objects are joined together as a linked list in the ``parameter_list:`` production. Join ``Parameter`` objects to the `end` of the linked list to preserve their ordering.
   
|

12. The action for ``object_declaration``'s first rule creates a game object and adds it to the symbol table. Append code after this to process the linked list of parameters (if any). Iterate thought the linked list of parameters: 
...
13. Ensure the project passes tests 9-12.

|

14. Add code to handle the other two rules in the ``variable:`` production. These
...
* Changed signature for new ``as_constant`` functions. 
* Provided default value to ``struct parameter``
